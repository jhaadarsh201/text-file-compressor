...
private:
    ...
    void internalInit();
    uint8_t * allocBytes(int bytesWanted, uint8_t * oldPtr, int oldSize);
    ...
    int numBitsWritten; //It is a global variable....jab bit add hua tab direct change 
                       //Number of bits in use from the stream buffer, not including byte-rounding padding.
    
public:   

    BitStreamWriter(const BitStreamWriter &) = delete; // -> It assures that BitStreamWriter class can't be  inherited
    BitStreamWriter & operator = (const BitStreamWriter &) = delete;

    BitStreamWriter();
    explicit BitStreamWriter(int initialSizeInBits, int growthGranularity = 2); //This is also the example of overloading
